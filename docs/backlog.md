...
- **v0.5:** Request-bound one-time approvals (cost/token digest + OIDC approver auth), production profile = PostgreSQL via Secret, single-node SQLite profile, Helm multi-replica SQLite guard, `/livez` + `/readyz`
