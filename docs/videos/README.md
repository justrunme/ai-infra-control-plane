# Product Videos

These H.264 MP4 walkthroughs complement the written architecture and keep the portfolio presentation concise. GitHub opens each file in its built-in media viewer.

| Video | Duration | Placement |
| --- | --- | --- |
| [Platform overview](hero-overview.mp4) | 10 seconds | Repository README |
| [Governance decision flow](governance-pipeline.mp4) | 10 seconds | Portfolio case study |
| [Digital twin](digital-twin.mp4) | 10 seconds | Platform architecture |
| [Forecast-driven scaling](forecast-driven-scaling.mp4) | 10 seconds | Platform architecture |
